...
Object.defineProperty(exports, "__esModule", ({ value: true }));
const cross_spawn_1 = __importDefault(__nccwpck_require__(746));
...
module.exports = __nccwpck_require__(927).default;
...
const niceTry = __nccwpck_require__(560);
...
var shebangRegex = __nccwpck_require__(638);
...
var COLON = isWindows ? ';' : ':'
var isexe = __nccwpck_require__(126)
...
const constants_PACKAGES_PATH = `${process.env.GITHUB_WORKSPACE}`;
const baseDir = process.env.GITHUB_WORKSPACE;
const releaseVersion = (/* unused pure expression or super */ null && (RELEASE_VERSION));
// Updates to only build 1 specific widget
const IDENTIFY_WIDGET_FOLDER = core.getInput('widget-folder');
const WORKSPACE_PATH = `${process.env.GITHUB_WORKSPACE}`;
const WIDGET_FOLDER = IDENTIFY_WIDGET_FOLDER;
const WIDGET_FOLDER_PATH = `${WORKSPACE_PATH}/${WIDGET_FOLDER}`;
...
var spawn_async = __nccwpck_require__(562);
...
const filesystemUtils_core = __nccwpck_require__(186);
...
function _readFileAsync(packagesPath) {
    return __awaiter(this, void 0, void 0, function* () {
        const foldersArray = yield fs.readdirSync(packagesPath, {
            withFileTypes: true,
        });
        return foldersArray;
    });
...
function getWidgetFolderStructure() {
    return {
...
const action_core = __nccwpck_require__(186);
// TODO
...
function buildWidget() {
    return action_awaiter(this, void 0, void 0, function* () {
        ...
        console.log(`Running action on path ${constants_PACKAGES_PATH}`);
        console.log(`Building widget in folder ${{ WIDGET_FOLDER: WIDGET_FOLDER }}`);
        console.log(`Building widget in path ${{ WIDGET_FOLDER_PATH: WIDGET_FOLDER_PATH }}`);
        // Builds a helper object with all paths that we will need
        const widgetFolderStructure = getWidgetFolderStructure();
        console.log(`Widget folder structure: ${widgetFolderStructure}`);
        // Reads package.json
        const packageJSON = yield _readPackageJSON(widgetFolderStructure);
        ...
        // Install packages
        try {
            yield runInstallCommand(widgetFolderStructure);
        }
        catch (error) {
            try {
        ...
        // Build new version
        try {
            yield runBuildCommand(widgetFolderStructure);
        }
        catch (error) {
